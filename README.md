...
### Django - 8000 порт
Web-приложение на Django. В нем доступна панель администратора в адресу /admin. Логин и пароль задается при разворачивании. В панели можно просмотреть результаты по задачам добавления в очередь и импорта в ClickHouse (Celery), создать пользователей и задачи.
...
События генерируются в Web интерфейсе. Результат можно просмотреть в Redash. На главной странцуе есть кнопки активации событий для пользователей. После их нажатия в течение 30 сек происходит импорт в БД. Просмотр БД возможен по адресу http://157.90.112.215 
