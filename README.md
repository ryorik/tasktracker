...
2. В менеджере задач (Celery) каждые пять минут происходит забор сообщений и импорт их в ClickHouse. В роли брокера сообщений для Celery используется Redis - разворачивается автоматически
